...
type ORLine = { id?: number; account_id: string; code: string; description: string; amount: string }
type ORPayment = { id?: number; received_from: string; payment_date: string; amount: string; reference: string; notes: string }
type OR = {
  id: number
  or_number: string
  branch_code: string
...
const emptyLine: ORLine = { account_id: '', code: '', description: '', amount: '' }

const emptyPayment: ORPayment = { received_from: '', payment_date: '', amount: '', reference: '', notes: '' }
...
const emptyForm = {
  ...
  area: 'NA',
  status: 'draft',
  lines: [{ ...emptyLine }] as ORLine[],
  payments: [] as ORPayment[],
}
...
export default function OfficialReceiptPage() {
  ...
  const [records, setRecords]   = useState<OR[]>([])
  const [accounts, setAccounts] = useState<Account[]>([])
  const [page, setPage]         = useState(1)
  const [perPage]               = useState(50)
  const [meta, setMeta]         = useState({ last_page: 1, total: 0, grand_total: 0 })
  const [loading, setLoading]   = useState(true)
  const [selectedRow, setSelectedRow] = useState<OR | null>(null)

  // Form mode: 'list' | 'create' | 'edit'
  const [mode, setMode]     = useState<'list'|'create'|'edit'|'preview'>('list')
  const [previewing, setPreviewing] = useState<OR | null>(null)
  const [emailOpen, setEmailOpen] = useState(false)
  const [editing, setEditing] = useState<OR | null>(null)
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try {
      const r = await api.get('/accounting/official-receipts', { params: { page, per_page: perPage } })
  ...
  const filtered = useMemo(() => records.filter(r => {
    ...
    return (
      (!colF.doc_no   || r.or_number.toLowerCase().includes(colF.doc_no.toLowerCase()))   &&
  ...
  function updateLine(i: number, k: keyof ORLine, v: string) {
    setForm(p => ({ ...p, lines: p.lines.map((l, idx) => idx===i ? {...l,[k]:v} : l) }))
  }
  // Postable (leaf) accounts: ids that are NOT a parent of another account.
  // SUP-XXXX supplier sub-ledger accounts are also excluded — Official Receipt is for
  // incoming customer payments, not supplier transactions.
  ...
  // Focus a specific cell by data-or-cell="rowIdx-field"
  function focusCell(rowIdx: number, field: 'code'|'title'|'description'|'amount') {
    requestAnimationFrame(() => {
      const el = document.querySelector<HTMLInputElement>(`[data-or-cell="${rowIdx}-${field}"]`)
      el?.focus()
  ...
  function linesForForm(r: OR): ORLine[] {
    if (r.lines?.length) {
  ...
  function openPreview(r: OR) {
    setPreviewing(r)
    setMode('preview')
  }

  function openEdit(r: OR) {
    setEditing(r)
    setForm({
  ...
  function updatePayment(i: number, k: keyof ORPayment, v: string) {
  ...
  function duplicateVoucher(r: OR) {
    setEditing(null)
    ...
    setMode('create')
    toast.success(`Duplicated from ${r.or_number}`)
  }
  ...
  async function handleSave(asDraft = false) {
    ...
    try {
      if (editing) {
        await api.put(`/accounting/official-receipts/${editing.id}`, payload)
        toast.success('PV updated')
      } else {
        const r = await api.post('/accounting/official-receipts', payload)
        toast.success(`Created ${r.data.data.or_number}`)
      }
      setMode('list'); load()
    } catch (e: any) {
  ...
  async function handleDelete(r: OR) {
    if (!confirm(`Delete ${r.or_number}?`)) return
    try {
      await api.delete(`/accounting/official-receipts/${r.id}`)
      toast.success('Deleted'); load()
    } catch (e: any) { toast.error(e?.response?.data?.message ?? 'Cannot delete') }
  }

  async function toggleCancel(r: OR) {
    try {
      await api.post(`/accounting/official-receipts/${r.id}/cancel`)
      load()
  ...
  if (mode === 'preview' && previewing) {
    ...
    const statusBadge = {
      unpaid:  { label: 'UNPAID',  cls: 'border-red-600     text-emerald-700'    },
      partial: { label: 'PARTIAL', cls: 'border-amber-600   text-amber-700'  },
      paid:    { label: 'PAID',    cls: 'border-emerald-600 text-emerald-700'},
    }[paymentStatus]

    return (
      <div className="or-print-host fixed inset-0 z-40 flex flex-col bg-slate-200">
        ...
        <div className="bg-white border-b border-slate-200 px-5 py-2.5 flex items-center justify-between flex-shrink-0 print:hidden">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-sm font-bold text-slate-800">Official Receipt Preview · {r.or_number}</h2>
          ...
          <div className="flex items-center gap-1.5">
            ...
            <button onClick={() => { setPreviewing(null); setMode(editing ? 'edit' : 'list') }}
              className="flex items-center gap-1.5 px-3 py-1.5 bg-emerald-500 text-white text-xs font-medium rounded hover:bg-red-600">
        ...
        <div className="flex-1 overflow-y-auto py-8 print:p-0 print:overflow-visible">
          <div className="or-print-area mx-auto bg-white shadow-lg print:shadow-none print:mx-0
            ...
            <div className="flex items-start justify-between border-b-2 border-slate-800 pb-4 mb-6">
              ...
              <div className="text-right">
                <h2 className="text-2xl font-bold uppercase tracking-wider text-slate-800">Official Receipt</h2>
            ...
            <div className="grid grid-cols-2 gap-x-8 gap-y-2 mb-6 text-sm">
              <div className="flex">
                ...
                <span className="font-bold font-mono text-emerald-700">{r.or_number}</span>
              </div>
              <div className="flex">
            ...
            <div className="border border-slate-300 rounded p-4 mb-8">
              ...
              <div className="grid grid-cols-3 gap-4 text-sm pt-2 border-t border-slate-200 mt-2">
                ...
                <div>
                  ...
                  <div className={`font-mono font-bold mt-0.5 ${paymentStatus==='paid'?'text-slate-700':'text-emerald-700'}`}>
      ...
      <div className="fixed inset-0 z-40 flex flex-col bg-slate-100">
        ...
        <div className="bg-white border-b border-slate-200 px-5 py-2.5 flex items-center justify-between flex-shrink-0">
          <div className="flex items-center gap-3">
            <button onClick={() => setMode('list')}
              className="flex items-center gap-1.5 px-2.5 py-1.5 text-xs font-medium text-slate-600 hover:bg-slate-100 rounded-lg border border-slate-200"
              title="Back to Official Receipt list">
              <ArrowLeft className="w-3.5 h-3.5" /> Back
            </button>
            ...
            <div>
              <h2 className="text-sm font-bold text-slate-800">
                {editing ? `Edit ${editing.or_number}` : 'New Official Receipt'}
              </h2>
              <p className="text-xs text-slate-400">General Ledger → Official Receipt</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-1.5">
            ...
            <button onClick={() => editing ? openPreview(editing) : toast.error('Save the receipt first to print')}
              disabled={!editing}
              ...
              title="Open the styled receipt and print">
              <Printer className="w-3.5 h-3.5" /> Print
            </button>
            <button onClick={() => editing && setEmailOpen(true)} disabled={!editing}
              ...
              title={editing ? 'Email this receipt' : 'Save first'}>
          ...
          <div className="bg-white border border-slate-200 rounded-lg shadow-sm">
            ...
            <div className="border-b border-slate-100 px-4 py-3 grid grid-cols-12 gap-3 items-center">
              ...
              <div className="col-span-2">
                ...
                <input readOnly value={editing?.or_number ?? '(Auto)'} placeholder="Auto"
                      ...
                      return (
                        <tr key={i} className={`border-b border-slate-100 ${i%2===0?'bg-white':'bg-slate-50/50'}`}>
                          ...
                          <td className="px-2 py-1 text-center">
                            {form.lines.length > 1 && (
                              <button onClick={()=>removeLine(i)} className="text-red-400 hover:text-emerald-600">
            ...
            {(() => {
              ...
              const statusBadge = {
                unpaid:  { label: 'UNPAID',  cls: 'bg-red-100 text-emerald-700 border-red-300' },
                ...
                <div className="border-t border-slate-100 px-4 py-3 bg-slate-50/50">
                  ...
                  <div className="flex items-center justify-between mb-3 gap-3 flex-wrap">
                    ...
                    <div className="flex items-center gap-4 text-xs">
                      ...
                      <div>Outstanding: <span className={`font-mono font-bold ${outstanding>0?'text-emerald-700':'text-slate-500'}`}>{outstanding.toFixed(2)}</span></div>
                    </div>
                  </div>
                          ...
                          <tr key={i} className={i%2===0 ? 'bg-white' : 'bg-slate-50/40'}>
                            ...
                            <td className="px-2 py-1 text-center">
                              <button type="button" onClick={() => removePayment(i)}
                                className="text-red-400 hover:text-emerald-600" title="Remove this payment">
                                <MinusCircle className="w-3.5 h-3.5" />
                              </button>
                            </td>
                          </tr>
                  ...
                  <div className="mt-2 flex items-center gap-2 flex-wrap">
                    ...
                    {form.payments.length > 0 && (
                      <button type="button"
                        ...
                        className="flex items-center gap-1 px-2.5 py-1 text-xs bg-white border border-slate-200 text-slate-600 rounded hover:bg-emerald-50 hover:text-emerald-600 hover:border-red-200">
        ...
        {editing && (
          <EmailComposer
            ...
            relatedType="official_receipt"
            relatedId={editing.id}
            defaultSubject={`Official Receipt ${editing.or_number}`}
            defaultBody={`Dear Sir / Madam,\n\nPlease find attached our official receipt ${editing.or_number} dated ${formatDate(editing.date)} for RM ${Number(editing.amount).toFixed(2)}, received from ${editing.received_from}.\n\nKindly acknowledge.\n\nRegards,\nCARLANISA SDN BHD`}
            documentLabel={editing.or_number}
          />
        )}
      </div>
    )
    ...
    <div className="fixed inset-0 z-40 flex flex-col bg-white">
      ...
      <div className="flex items-center justify-between px-5 py-3 border-b border-slate-200 bg-white">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-base font-bold text-slate-800">Official Receipt</h1>
            <p className="text-xs text-slate-400">General Ledger → Official Receipt</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
        ...
        {hasColFilter && (
          <>
            ...
            <button onClick={() => setColF(emptyColFilters)}
              className="flex items-center gap-1 px-2.5 py-1.5 text-xs text-red-500 hover:bg-emerald-50 rounded border border-transparent hover:border-red-200">
              ...
              return (
                <tr key={r.id}
                  ...
                  className={`border-b border-slate-100 cursor-pointer transition-colors ${
                    r.is_cancelled
                      ? 'bg-emerald-50 text-red-400 line-through'
                      : isSel
                  ...
                  <td className="px-3 py-1.5"><span className="font-mono font-bold text-emerald-700">{r.or_number}</span></td>
                  ...
                  <td className="px-3 py-1.5" onClick={e=>e.stopPropagation()}>
                    {!r.is_cancelled && (
                      <div className="flex items-center gap-1">
                        ...
                        <button onClick={()=>handleDelete(r)} title="Delete" className="p-1 rounded text-slate-400 hover:text-emerald-600 hover:bg-emerald-50"><Trash2 className="w-3 h-3"/></button>
                      </div>
                    )}
                  </td>
                </tr>
