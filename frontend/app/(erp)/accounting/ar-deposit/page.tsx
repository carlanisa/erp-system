...
type ARDLine = { id?: number; account_id: string; code: string; description: string; amount: string }
type ARDPayment = { id?: number; received_from: string; payment_date: string; amount: string; reference: string; notes: string }
type ARD = {
  id: number
  ...
  payments?: { id: number; received_from: string | null; payment_date: string; amount: number; reference: string | null; notes: string | null }[]
}
...
const emptyLine: ARDLine = { account_id: '', code: '', description: '', amount: '' }

const emptyPayment: ARDPayment = { received_from: '', payment_date: '', amount: '', reference: '', notes: '' }
...
const emptyForm = {
  ...
  area: 'NA',
  status: 'draft',
  lines: [{ ...emptyLine }] as ARDLine[],
  payments: [] as ARDPayment[],
}
...
export default function ArDepositPage() {
  const router = useRouter()
  const [records, setRecords]   = useState<ARD[]>([])
  const [accounts, setAccounts] = useState<Account[]>([])
  const [page, setPage]         = useState(1)
  const [perPage]               = useState(50)
  const [meta, setMeta]         = useState({ last_page: 1, total: 0, grand_total: 0 })
  const [loading, setLoading]   = useState(true)
  const [selectedRow, setSelectedRow] = useState<ARD | null>(null)

  // Form mode: 'list' | 'create' | 'edit'
  const [mode, setMode]     = useState<'list'|'create'|'edit'|'preview'>('list')
  const [previewing, setPreviewing] = useState<ARD | null>(null)
  const [emailOpen, setEmailOpen] = useState(false)
  const [editing, setEditing] = useState<ARD | null>(null)
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try {
      const r = await api.get('/accounting/ar-deposits', { params: { page, per_page: perPage } })
  ...
  const filtered = useMemo(() => records.filter(r => {
    ...
    return (
      (!colF.doc_no   || r.deposit_number.toLowerCase().includes(colF.doc_no.toLowerCase()))   &&
      ...
      (!colF.pay_to   || r.received_from.toLowerCase().includes(colF.pay_to.toLowerCase()))        &&
  ...
  function updateLine(i: number, k: keyof ARDLine, v: string) {
    setForm(p => ({ ...p, lines: p.lines.map((l, idx) => idx===i ? {...l,[k]:v} : l) }))
  }
  // Postable (leaf) accounts: ids that are NOT a parent of another account.
  // SUP-XXXX supplier sub-ledger accounts are also excluded — A/R Deposit is for
  // customer advance receipts, not supplier transactions.
  ...
  // Focus a specific cell by data-ard-cell="rowIdx-field"
  function focusCell(rowIdx: number, field: 'code'|'title'|'description'|'amount') {
    requestAnimationFrame(() => {
      const el = document.querySelector<HTMLInputElement>(`[data-ard-cell="${rowIdx}-${field}"]`)
      el?.focus()
  ...
  function linesForForm(r: ARD): ARDLine[] {
    if (r.lines?.length) {
  ...
  function openPreview(r: ARD) {
    setPreviewing(r)
    setMode('preview')
  }

  function openEdit(r: ARD) {
    setEditing(r)
    setForm({
      ...
      payments: (r.payments ?? []).map(p => ({
        id: p.id,
        received_from: p.received_from ?? r.received_from ?? '',
  ...
  function addPayment() {
    setForm(p => ({
      ...
      payments: [...p.payments, {
        ...emptyPayment,
        received_from: p.received_from,                                          // default to voucher's Received From
  ...
  function updatePayment(i: number, k: keyof ARDPayment, v: string) {
  ...
  function duplicateVoucher(r: ARD) {
    setEditing(null)
    ...
    setMode('create')
    toast.success(`Duplicated from ${r.deposit_number}`)
  }
  ...
  async function handleSave(asDraft = false) {
    if (!form.received_from) { toast.error('Received From is required'); return }
    ...
    const payload = {
      ...
      payments: form.payments.map(p => ({
        received_from: p.received_from || form.received_from,                       // default to voucher received_from on save
    ...
    try {
      if (editing) {
        await api.put(`/accounting/ar-deposits/${editing.id}`, payload)
        toast.success('PV updated')
      } else {
        const r = await api.post('/accounting/ar-deposits', payload)
        toast.success(`Created ${r.data.data.deposit_number}`)
      }
      setMode('list'); load()
    } catch (e: any) {
  ...
  async function handleDelete(r: ARD) {
    if (!confirm(`Delete ${r.deposit_number}?`)) return
    try {
      await api.delete(`/accounting/ar-deposits/${r.id}`)
      toast.success('Deleted'); load()
    } catch (e: any) { toast.error(e?.response?.data?.message ?? 'Cannot delete') }
  }

  async function toggleCancel(r: ARD) {
    try {
      await api.post(`/accounting/ar-deposits/${r.id}/cancel`)
      load()
  ...
  if (mode === 'preview' && previewing) {
    ...
    const statusBadge = {
      unpaid:  { label: 'UNPAID',  cls: 'border-red-600     text-sky-700'    },
      partial: { label: 'PARTIAL', cls: 'border-amber-600   text-amber-700'  },
      paid:    { label: 'PAID',    cls: 'border-emerald-600 text-sky-700'},
    }[paymentStatus]

    return (
      <div className="ard-print-host fixed inset-0 z-40 flex flex-col bg-slate-200">
        ...
        <div className="bg-white border-b border-slate-200 px-5 py-2.5 flex items-center justify-between flex-shrink-0 print:hidden">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-sm font-bold text-slate-800">A/R Deposit Preview · {r.deposit_number}</h2>
          ...
          <div className="flex items-center gap-1.5">
            ...
            <button onClick={() => { setPreviewing(null); setMode(editing ? 'edit' : 'list') }}
              className="flex items-center gap-1.5 px-3 py-1.5 bg-sky-500 text-white text-xs font-medium rounded hover:bg-red-600">
        ...
        <div className="flex-1 overflow-y-auto py-8 print:p-0 print:overflow-visible">
          <div className="ard-print-area mx-auto bg-white shadow-lg print:shadow-none print:mx-0
            ...
            <div className="flex items-start justify-between border-b-2 border-slate-800 pb-4 mb-6">
              ...
              <div className="text-right">
                <h2 className="text-2xl font-bold uppercase tracking-wider text-slate-800">A/R Deposit</h2>
            ...
            <div className="grid grid-cols-2 gap-x-8 gap-y-2 mb-6 text-sm">
              <div className="flex">
                ...
                <span className="font-bold font-mono text-sky-700">{r.deposit_number}</span>
              </div>
              <div className="flex">
            ...
            {/* Received From & Payment By panel */}
            <div className="grid grid-cols-2 gap-4 mb-6">
              <div className="border border-slate-300 rounded p-3">
                <div className="text-[11px] font-semibold uppercase tracking-wide text-slate-500 mb-1">Received From</div>
                <div className="text-base font-bold text-slate-900">{r.received_from}</div>
              </div>
              <div className="border border-slate-300 rounded p-3">
            ...
            <div className="border border-slate-300 rounded p-4 mb-8">
                ...
                <table className="w-full text-xs border-collapse mb-2">
                  <thead>
                    <tr className="bg-slate-100 border-b border-slate-300">
                      ...
                      <th className="px-2 py-1.5 text-left font-semibold">Received From</th>
                  ...
                  <tbody>
                    {r.payments.map((p, i) => (
                      <tr key={p.id} className="border-b border-slate-200">
                        ...
                        <td className="px-2 py-1.5 font-medium">{p.received_from || r.received_from}</td>
              ...
              <div className="grid grid-cols-3 gap-4 text-sm pt-2 border-t border-slate-200 mt-2">
                ...
                <div>
                  ...
                  <div className="font-mono font-semibold mt-0.5 text-sky-700">RM {(r.paid_amount ?? 0).toFixed(2)}</div>
                </div>
                <div>
                  <div className="text-slate-500 text-xs">Outstanding</div>
                  <div className={`font-mono font-bold mt-0.5 ${paymentStatus==='paid'?'text-slate-700':'text-sky-700'}`}>
      ...
      <div className="fixed inset-0 z-40 flex flex-col bg-slate-100">
        ...
        <div className="bg-white border-b border-slate-200 px-5 py-2.5 flex items-center justify-between flex-shrink-0">
          <div className="flex items-center gap-3">
            <button onClick={() => setMode('list')}
              className="flex items-center gap-1.5 px-2.5 py-1.5 text-xs font-medium text-slate-600 hover:bg-slate-100 rounded-lg border border-slate-200"
              title="Back to A/R Deposit list">
              <ArrowLeft className="w-3.5 h-3.5" /> Back
            </button>
            <div className="w-8 h-8 bg-sky-50 rounded-lg flex items-center justify-center">
              <FileOutput className="w-4 h-4 text-sky-600" />
            </div>
            <div>
              <h2 className="text-sm font-bold text-slate-800">
                {editing ? `Edit ${editing.deposit_number}` : 'New A/R Deposit'}
              </h2>
              <p className="text-xs text-slate-400">General Ledger → A/R Deposit</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-1.5">
            ...
            <button onClick={() => editing ? openPreview(editing) : toast.error('Save the deposit first to print')}
              disabled={!editing}
              ...
              title="Open the styled deposit and print">
              <Printer className="w-3.5 h-3.5" /> Print
            </button>
            <button onClick={() => editing && setEmailOpen(true)} disabled={!editing}
              ...
              title={editing ? 'Email this deposit' : 'Save first'}>
          ...
          <div className="bg-white border border-slate-200 rounded-lg shadow-sm">
            ...
            <div className="border-b border-slate-100 px-4 py-3 grid grid-cols-12 gap-3 items-center">
              ...
              <div className="col-span-2">
                ...
                <input readOnly value={editing?.deposit_number ?? '(Auto)'} placeholder="Auto"
              ...
              <div className="col-span-2">
                <label className="block text-[11px] font-semibold text-slate-500 mb-1 uppercase tracking-wide">Posting Date</label>
                <input type="date" value={form.posting_date ?? ''} onChange={e=>sf('posting_date',e.target.value)}
                  className="w-full px-2.5 py-1.5 text-sm border border-slate-300 rounded focus:outline-none focus:border-indigo-400" />
              </div>
            </div>
            ...
            <div className="border-b border-slate-100 px-4 py-3 flex items-center gap-3">
              <label className="text-sm font-semibold text-slate-700 w-28 flex-shrink-0">Received From</label>
              <input value={form.received_from} onChange={e=>sf('received_from',e.target.value)}
            ...
            <div className="border-b border-slate-200">
              <div className="flex gap-0 px-4 pt-2">
                {['Main','External Document','Note'].map((t,i)=>(
                  <div key={t} className={`px-4 py-1.5 text-xs font-medium border-b-2 cursor-pointer ${i===0?'border-indigo-600 text-indigo-700 bg-white':'border-transparent text-slate-500 hover:text-slate-700'}`}>
                    {t}
                  </div>
                ))}
              </div>
                      ...
                      return (
                        <tr key={i} className={`border-b border-slate-100 ${i%2===0?'bg-white':'bg-slate-50/50'}`}>
                          ...
                          <td className="px-2 py-1 text-center">
                            {form.lines.length > 1 && (
                              <button onClick={()=>removeLine(i)} className="text-red-400 hover:text-sky-600">
            ...
            {(() => {
              ...
              const statusBadge = {
                unpaid:  { label: 'UNPAID',  cls: 'bg-red-100 text-sky-700 border-red-300' },
                partial: { label: 'PARTIAL', cls: 'bg-amber-100 text-amber-700 border-amber-300' },
                paid:    { label: 'PAID',    cls: 'bg-emerald-100 text-sky-700 border-emerald-300' },
              }[status]
                ...
                <div className="border-t border-slate-100 px-4 py-3 bg-slate-50/50">
                  ...
                  <div className="flex items-center justify-between mb-3 gap-3 flex-wrap">
                    ...
                    <div className="flex items-center gap-4 text-xs">
                      ...
                      <div>Paid: <span className="font-mono font-semibold text-sky-700">{paidSum.toFixed(2)}</span></div>
                      <div>Outstanding: <span className={`font-mono font-bold ${outstanding>0?'text-sky-700':'text-slate-500'}`}>{outstanding.toFixed(2)}</span></div>
                    </div>
                  </div>
                    ...
                    <table className="w-full text-xs">
                      ...
                      <tbody>
                        {form.payments.length === 0 ? (
                          <tr>
                            <td colSpan={7} className="px-3 py-4 text-center text-slate-400 text-xs italic">
                          ...
                          <tr key={i} className={i%2===0 ? 'bg-white' : 'bg-slate-50/40'}>
                            ...
                            <td className="px-1.5 py-1">
                              <input value={p.received_from}
                                onChange={e => updatePayment(i, 'received_from', e.target.value)}
                                placeholder={form.received_from || 'Payee name'}
                            ...
                            <td className="px-2 py-1 text-center">
                              <button type="button" onClick={() => removePayment(i)}
                                className="text-red-400 hover:text-sky-600" title="Remove this payment">
                                <MinusCircle className="w-3.5 h-3.5" />
                              </button>
                            </td>
                          </tr>
                  ...
                  <div className="mt-2 flex items-center gap-2 flex-wrap">
                    ...
                    <button type="button"
                      onClick={() => {
                        ...
                        setForm(p => ({
                          ...p,
                          payments: [...p.payments, {
                            received_from: p.received_from, payment_date: today, amount: outstanding.toFixed(2), reference: '', notes: '',
                          }],
                        }))
                      }}
                    ...
                    {form.payments.length > 0 && (
                      <button type="button"
                        ...
                        className="flex items-center gap-1 px-2.5 py-1 text-xs bg-white border border-slate-200 text-slate-600 rounded hover:bg-sky-50 hover:text-sky-600 hover:border-red-200">
        ...
        {editing && (
          <EmailComposer
            ...
            relatedType="ar_deposit"
            relatedId={editing.id}
            defaultSubject={`A/R Deposit ${editing.deposit_number}`}
            defaultBody={`Dear Sir / Madam,\n\nPlease find attached our A/R deposit ${editing.deposit_number} dated ${formatDate(editing.date)} for RM ${Number(editing.amount).toFixed(2)}.\n\nRegards,\nCARLANISA SDN BHD`}
            documentLabel={editing.deposit_number}
          />
        )}
      </div>
    )
      ...
      <div className="flex items-center justify-between px-5 py-3 border-b border-slate-200 bg-white">
        <div className="flex items-center gap-3">
          ...
          <div className="w-9 h-9 bg-sky-50 rounded-xl flex items-center justify-center">
            <FileOutput className="w-5 h-5 text-sky-600" />
          </div>
          <div>
            <h1 className="text-base font-bold text-slate-800">A/R Deposit</h1>
            <p className="text-xs text-slate-400">General Ledger → A/R Deposit</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
        ...
        {hasColFilter && (
          <>
            ...
            <button onClick={() => setColF(emptyColFilters)}
              className="flex items-center gap-1 px-2.5 py-1.5 text-xs text-red-500 hover:bg-sky-50 rounded border border-transparent hover:border-red-200">
        ...
        <table className="w-full text-xs border-collapse">
          <thead>
            <tr className="bg-slate-700 text-white">
              ...
              <th className="text-left px-3 py-2 font-semibold whitespace-nowrap">Received From</th>
              ...
              return (
                <tr key={r.id}
                  ...
                  className={`border-b border-slate-100 cursor-pointer transition-colors ${
                    r.is_cancelled
                      ? 'bg-sky-50 text-red-400 line-through'
                      : isSel
                  ...
                  <td className="px-3 py-1.5"><span className="font-mono font-bold text-sky-700">{r.deposit_number}</span></td>
                  ...
                  <td className="px-3 py-1.5 text-slate-800 font-medium max-w-[220px] truncate" title={r.received_from}>{r.received_from}</td>
                  ...
                  <td className="px-3 py-1.5 text-center">
                    {(() => {
                      ...
                      const cls = ps==='paid'    ? 'bg-emerald-100 text-sky-700'
                                : ps==='partial' ? 'bg-amber-100 text-amber-700'
                                                 : 'bg-red-100 text-sky-700'
                  ...
                  <td className="px-3 py-1.5" onClick={e=>e.stopPropagation()}>
                    {!r.is_cancelled && (
                      <div className="flex items-center gap-1">
                        ...
                        <button onClick={()=>duplicateVoucher(r)} title="Duplicate" className="p-1 rounded text-slate-400 hover:text-sky-600 hover:bg-sky-50"><Copy className="w-3 h-3"/></button>
                        <button onClick={()=>handleDelete(r)} title="Delete" className="p-1 rounded text-slate-400 hover:text-sky-600 hover:bg-sky-50"><Trash2 className="w-3 h-3"/></button>
                      </div>
                    )}
                  </td>
                </tr>
