...
type APDLine = { id?: number; account_id: string; code: string; description: string; amount: string }
type APDPayment = { id?: number; supplier_name: string; payment_date: string; amount: string; reference: string; notes: string }
type APD = {
  id: number
  ...
  payments?: { id: number; supplier_name: string | null; payment_date: string; amount: number; reference: string | null; notes: string | null }[]
}
...
const emptyLine: APDLine = { account_id: '', code: '', description: '', amount: '' }

const emptyPayment: APDPayment = { supplier_name: '', payment_date: '', amount: '', reference: '', notes: '' }
...
const emptyForm = {
  ...
  area: 'NA',
  status: 'draft',
  lines: [{ ...emptyLine }] as APDLine[],
  payments: [] as APDPayment[],
}
...
export default function ApDepositPage() {
  const router = useRouter()
  const [records, setRecords]   = useState<APD[]>([])
  const [accounts, setAccounts] = useState<Account[]>([])
  const [page, setPage]         = useState(1)
  const [perPage]               = useState(50)
  const [meta, setMeta]         = useState({ last_page: 1, total: 0, grand_total: 0 })
  const [loading, setLoading]   = useState(true)
  const [selectedRow, setSelectedRow] = useState<APD | null>(null)

  // Form mode: 'list' | 'create' | 'edit'
  const [mode, setMode]     = useState<'list'|'create'|'edit'|'preview'>('list')
  const [previewing, setPreviewing] = useState<APD | null>(null)
  const [emailOpen, setEmailOpen] = useState(false)
  const [editing, setEditing] = useState<APD | null>(null)
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try {
      const r = await api.get('/accounting/ap-deposits', { params: { page, per_page: perPage } })
  ...
  const filtered = useMemo(() => records.filter(r => {
    ...
    return (
      ...
      (!colF.pay_to   || r.supplier_name.toLowerCase().includes(colF.pay_to.toLowerCase()))        &&
  ...
  function updateLine(i: number, k: keyof APDLine, v: string) {
  ...
  // Focus a specific cell by data-apd-cell="rowIdx-field"
  function focusCell(rowIdx: number, field: 'code'|'title'|'description'|'amount') {
    requestAnimationFrame(() => {
      const el = document.querySelector<HTMLInputElement>(`[data-apd-cell="${rowIdx}-${field}"]`)
      el?.focus()
  ...
  function linesForForm(r: APD): APDLine[] {
    if (r.lines?.length) {
  ...
  function openPreview(r: APD) {
    setPreviewing(r)
    setMode('preview')
  }

  function openEdit(r: APD) {
    setEditing(r)
    setForm({
      ...
      payments: (r.payments ?? []).map(p => ({
        id: p.id,
        supplier_name: p.supplier_name ?? r.supplier_name ?? '',
  ...
  function addPayment() {
    setForm(p => ({
      ...
      payments: [...p.payments, {
        ...emptyPayment,
        supplier_name: p.supplier_name,                                          // default to voucher's Supplier
  ...
  function updatePayment(i: number, k: keyof APDPayment, v: string) {
  ...
  function duplicateVoucher(r: APD) {
    setEditing(null)
  ...
  async function handleSave(asDraft = false) {
    ...
    const payload = {
      ...
      payments: form.payments.map(p => ({
        supplier_name: p.supplier_name || form.supplier_name,                       // default to voucher supplier_name on save
    ...
    try {
      if (editing) {
        await api.put(`/accounting/ap-deposits/${editing.id}`, payload)
        toast.success('PV updated')
      } else {
        const r = await api.post('/accounting/ap-deposits', payload)
  ...
  async function handleDelete(r: APD) {
    if (!confirm(`Delete ${r.deposit_number}?`)) return
    try {
      await api.delete(`/accounting/ap-deposits/${r.id}`)
      toast.success('Deleted'); load()
    } catch (e: any) { toast.error(e?.response?.data?.message ?? 'Cannot delete') }
  }

  async function toggleCancel(r: APD) {
    try {
      await api.post(`/accounting/ap-deposits/${r.id}/cancel`)
      load()
  ...
  if (mode === 'preview' && previewing) {
    ...
    const statusBadge = {
      unpaid:  { label: 'UNPAID',  cls: 'border-red-600     text-orange-700'    },
      partial: { label: 'PARTIAL', cls: 'border-amber-600   text-amber-700'  },
      paid:    { label: 'PAID',    cls: 'border-emerald-600 text-emerald-700'},
    }[paymentStatus]

    return (
      <div className="apd-print-host fixed inset-0 z-40 flex flex-col bg-slate-200">
        ...
        <div className="bg-white border-b border-slate-200 px-5 py-2.5 flex items-center justify-between flex-shrink-0 print:hidden">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-sm font-bold text-slate-800">A/P Deposit Preview · {r.deposit_number}</h2>
          ...
          <div className="flex items-center gap-1.5">
            ...
            <button onClick={() => { setPreviewing(null); setMode(editing ? 'edit' : 'list') }}
              className="flex items-center gap-1.5 px-3 py-1.5 bg-orange-500 text-white text-xs font-medium rounded hover:bg-red-600">
        ...
        <div className="flex-1 overflow-y-auto py-8 print:p-0 print:overflow-visible">
          <div className="apd-print-area mx-auto bg-white shadow-lg print:shadow-none print:mx-0
            ...
            <div className="flex items-start justify-between border-b-2 border-slate-800 pb-4 mb-6">
              ...
              <div className="text-right">
                <h2 className="text-2xl font-bold uppercase tracking-wider text-slate-800">A/P Deposit</h2>
            ...
            <div className="grid grid-cols-2 gap-x-8 gap-y-2 mb-6 text-sm">
              <div className="flex">
                ...
                <span className="font-bold font-mono text-orange-700">{r.deposit_number}</span>
              </div>
              <div className="flex">
            ...
            <div className="grid grid-cols-2 gap-4 mb-6">
              <div className="border border-slate-300 rounded p-3">
                ...
                <div className="text-base font-bold text-slate-900">{r.supplier_name}</div>
              </div>
              <div className="border border-slate-300 rounded p-3">
            ...
            <div className="border border-slate-300 rounded p-4 mb-8">
                  ...
                  <tbody>
                    {r.payments.map((p, i) => (
                      <tr key={p.id} className="border-b border-slate-200">
                        ...
                        <td className="px-2 py-1.5 font-medium">{p.supplier_name || r.supplier_name}</td>
              ...
              <div className="grid grid-cols-3 gap-4 text-sm pt-2 border-t border-slate-200 mt-2">
                ...
                <div>
                  ...
                  <div className={`font-mono font-bold mt-0.5 ${paymentStatus==='paid'?'text-slate-700':'text-orange-700'}`}>
      ...
      <div className="fixed inset-0 z-40 flex flex-col bg-slate-100">
        ...
        <div className="bg-white border-b border-slate-200 px-5 py-2.5 flex items-center justify-between flex-shrink-0">
          <div className="flex items-center gap-3">
            <button onClick={() => setMode('list')}
              className="flex items-center gap-1.5 px-2.5 py-1.5 text-xs font-medium text-slate-600 hover:bg-slate-100 rounded-lg border border-slate-200"
              title="Back to A/P Deposit list">
              <ArrowLeft className="w-3.5 h-3.5" /> Back
            </button>
            ...
            <div>
              <h2 className="text-sm font-bold text-slate-800">
                {editing ? `Edit ${editing.deposit_number}` : 'New A/P Deposit'}
              </h2>
              <p className="text-xs text-slate-400">General Ledger → A/P Deposit</p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-4">
          <div className="bg-white border border-slate-200 rounded-lg shadow-sm">
            ...
            <div className="border-b border-slate-100 px-4 py-3 flex items-center gap-3">
              <label className="text-sm font-semibold text-slate-700 w-28 flex-shrink-0">Supplier</label>
              <input value={form.supplier_name} onChange={e=>sf('supplier_name',e.target.value)}
                      ...
                      return (
                        <tr key={i} className={`border-b border-slate-100 ${i%2===0?'bg-white':'bg-slate-50/50'}`}>
                          ...
                          <td className="px-2 py-1 text-center">
                            {form.lines.length > 1 && (
                              <button onClick={()=>removeLine(i)} className="text-red-400 hover:text-orange-600">
            ...
            {(() => {
              ...
              const statusBadge = {
                unpaid:  { label: 'UNPAID',  cls: 'bg-red-100 text-orange-700 border-red-300' },
                ...
                <div className="border-t border-slate-100 px-4 py-3 bg-slate-50/50">
                  ...
                  <div className="flex items-center justify-between mb-3 gap-3 flex-wrap">
                    ...
                    <div className="flex items-center gap-4 text-xs">
                      ...
                      <div>Outstanding: <span className={`font-mono font-bold ${outstanding>0?'text-orange-700':'text-slate-500'}`}>{outstanding.toFixed(2)}</span></div>
                    </div>
                  </div>
                    ...
                    <table className="w-full text-xs">
                      <thead>
                        <tr className="bg-slate-100 border-b border-slate-200">
                          ...
                          <th className="px-3 py-1.5 text-left font-semibold text-slate-600 w-44">Supplier</th>
                      ...
                      <tbody>
                        {form.payments.length === 0 ? (
                          ...
                          <tr key={i} className={i%2===0 ? 'bg-white' : 'bg-slate-50/40'}>
                            ...
                            <td className="px-1.5 py-1">
                              <input value={p.supplier_name}
                                onChange={e => updatePayment(i, 'supplier_name', e.target.value)}
                                placeholder={form.supplier_name || 'Payee name'}
                            ...
                            <td className="px-2 py-1 text-center">
                              <button type="button" onClick={() => removePayment(i)}
                                className="text-red-400 hover:text-orange-600" title="Remove this payment">
                                <MinusCircle className="w-3.5 h-3.5" />
                              </button>
                            </td>
                          </tr>
                  ...
                  <div className="mt-2 flex items-center gap-2 flex-wrap">
                    ...
                    <button type="button"
                      onClick={() => {
                        ...
                        setForm(p => ({
                          ...p,
                          payments: [...p.payments, {
                            supplier_name: p.supplier_name, payment_date: today, amount: outstanding.toFixed(2), reference: '', notes: '',
                          }],
                        }))
                      }}
                    ...
                    {form.payments.length > 0 && (
                      <button type="button"
                        ...
                        className="flex items-center gap-1 px-2.5 py-1 text-xs bg-white border border-slate-200 text-slate-600 rounded hover:bg-orange-50 hover:text-orange-600 hover:border-red-200">
        ...
        {editing && (
          <EmailComposer
            ...
            relatedType="ap_deposit"
            relatedId={editing.id}
            defaultSubject={`A/P Deposit ${editing.deposit_number}`}
            defaultBody={`Dear Sir / Madam,\n\nPlease find attached our A/P deposit ${editing.deposit_number} dated ${formatDate(editing.date)} for RM ${Number(editing.amount).toFixed(2)}, paid to ${editing.supplier_name}.\n\nRegards,\nCARLANISA SDN BHD`}
    ...
    <div className="fixed inset-0 z-40 flex flex-col bg-white">
      ...
      <div className="flex items-center justify-between px-5 py-3 border-b border-slate-200 bg-white">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-base font-bold text-slate-800">A/P Deposit</h1>
            <p className="text-xs text-slate-400">General Ledger → A/P Deposit</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
        ...
        {hasColFilter && (
          <>
            ...
            <button onClick={() => setColF(emptyColFilters)}
              className="flex items-center gap-1 px-2.5 py-1.5 text-xs text-red-500 hover:bg-orange-50 rounded border border-transparent hover:border-red-200">
              ...
              return (
                <tr key={r.id}
                  ...
                  className={`border-b border-slate-100 cursor-pointer transition-colors ${
                    r.is_cancelled
                      ? 'bg-orange-50 text-red-400 line-through'
                      : isSel
                  ...
                  <td className="px-3 py-1.5"><span className="font-mono font-bold text-orange-700">{r.deposit_number}</span></td>
                  ...
                  <td className="px-3 py-1.5 text-slate-800 font-medium max-w-[220px] truncate" title={r.supplier_name}>{r.supplier_name}</td>
                  ...
                  <td className="px-3 py-1.5" onClick={e=>e.stopPropagation()}>
                    {!r.is_cancelled && (
                      <div className="flex items-center gap-1">
                        ...
                        <button onClick={()=>handleDelete(r)} title="Delete" className="p-1 rounded text-slate-400 hover:text-orange-600 hover:bg-orange-50"><Trash2 className="w-3 h-3"/></button>
                      </div>
                    )}
                  </td>
                </tr>
